//! Tuned Tokio implementations of every workload, shared by two binaries that
//! differ only in the channels of the channel tests (`Channels`):
//! `tokio-tuned-bench` uses kanal, `tokio-tuned-tokio-channels-bench` uses
//! Tokio's own mpsc (async-channel for MPMC, as `tokio-bench` does: Tokio has no
//! multi-receiver channel), to isolate what kanal contributes.
//!
...
//! - kanal channels instead of tokio mpsc / async-channel (spsc, mpsc, mpmc, pingpong; `Channels::Kanal` only)
...
/// The channels the channel tests use. Everything else is the same code in both binaries.
#[derive(Clone, Copy)]
pub enum Channels {
    /// kanal: `tokio-tuned-bench`.
    Kanal,
    /// Tokio's own mpsc, and async-channel for MPMC: `tokio-tuned-tokio-channels-bench`.
    Tokio,
}

/// The implementation name reported in every result, set once by `main`.
static IMPL: OnceLock<&'static str> = OnceLock::new();

fn impl_name() -> &'static str {
    IMPL.get().expect("main sets IMPL")
}

/// Most messages a consumer takes per wake-up.
const BATCH: usize = 256;

/// Runs one workload and prints its report. Each binary's `main` calls this; the binaries set mimalloc as
/// their global allocator.
pub fn main(channels: Channels) {
    IMPL.set(match channels {
        Channels::Kanal => "tokio-tuned",
        Channels::Tokio => "tokio-tuned-tokio-channels",
    })
    .expect("main runs once");
    let args = Args::parse();
    ...
    let report = rt.block_on(async move { tokio::spawn(run(args, channels)).await.expect("workload task panicked") });
    report.print();
}

async fn run(a: Args, channels: Channels) -> Report {
    match (a.workload.as_str(), channels) {
        ("spsc", Channels::Kanal) => spsc(a).await,
        ("spsc", Channels::Tokio) => spsc_mpsc(a).await,
        ("mpmc", Channels::Kanal) => mpmc(a).await,
        ("mpmc", Channels::Tokio) => mpmc_async_channel(a).await,
        ("mpsc", Channels::Kanal) => many_to_one(a).await,
        ("mpsc", Channels::Tokio) => many_to_one_mpsc(a).await,
        ("pingpong", Channels::Kanal) => pingpong(a).await,
        ("pingpong", Channels::Tokio) => pingpong_mpsc(a).await,
        ("spawn", _) => spawn(a).await,
        ("cpu", _) => cpu(a).await,
        ("select", _) => select(a).await,
        ("mutex", _) => mutex(a).await,
        ("idle", _) => idle(a).await,
        (other, _) => fail(&format!("unknown workload {other}")),
    }
}
...
async fn spsc(a: Args) -> Report {
    ...
    Report::ok(impl_name(), &a, n, start.elapsed(), sum)
}
...
async fn many_to_one(a: Args) -> Report {
    ...
    let sum = consumer.await.unwrap();
    Report::ok(impl_name(), &a, total, start.elapsed(), sum)
}
...
async fn pingpong(a: Args) -> Report {
    ...
    b.await.unwrap();
    Report::ok(impl_name(), &a, n, start.elapsed(), token).with_latencies(&mut samples)
}
...
async fn spawn(a: Args) -> Report {
    ...
    Report::ok(impl_name(), &a, n, elapsed, sum)
}
...
async fn select(a: Args) -> Report {
    ...
    p2.await.unwrap();
    let sum = consumer.await.unwrap();
    Report::ok(impl_name(), &a, 2 * half, start.elapsed(), sum)
}
...
async fn mutex(a: Args) -> Report {
    ...
    Report::ok(impl_name(), &a, per * a.workers as u64, elapsed, total)
}
...
async fn idle(a: Args) -> Report {
    ...
    Report::ok(impl_name(), &a, n, elapsed, finished.load(Ordering::Relaxed)).with_memory(before, after, n)
}
...
// ---- Channel tests on Tokio's own mpsc (`Channels::Tokio`)

/// `drain_sum` on a tokio mpsc receiver.
async fn drain_sum_mpsc(mut rx: mpsc::Receiver<u64>) -> u64 {
    let mut sum = 0u64;
...
async fn spsc_mpsc(a: Args) -> Report {
    let n = a.size;
    let (tx, rx) = mpsc::channel::<u64>(a.capacity);
    let start = Instant::now();
    let producer = tokio::spawn(produce_mpsc(tx, 0..n));
    let consumer = tokio::spawn(drain_sum_mpsc(rx));
    producer.await.unwrap();
    let sum = consumer.await.unwrap();
    Report::ok(impl_name(), &a, n, start.elapsed(), sum)
}
...
/// Several senders, one receiver.
async fn many_to_one_mpsc(a: Args) -> Report {
    let per = a.size / a.producers as u64;
    let total = per * a.producers as u64;
    let (tx, rx) = mpsc::channel::<u64>(a.capacity);
    let mut producers = Vec::with_capacity(a.producers);
    ...
    let consumer = tokio::spawn(drain_sum_mpsc(rx));
    for p in 0..a.producers as u64 {
        producers.push(tokio::spawn(produce_mpsc(tx.clone(), p * per..(p + 1) * per)));
    }
    drop(tx);
    for p in producers {
        p.await.unwrap();
...
async fn pingpong_mpsc(a: Args) -> Report {
    let n = a.size;
    let every = a.sample_every;
    ...
    let samples = Vec::with_capacity((n / every + 1) as usize);
    let start = Instant::now();
    let b = tokio::spawn(pong_mpsc(b_rx, to_a));
    let a_side = tokio::spawn(ping_mpsc(to_b, a_rx, n, every, samples));
    let (token, mut samples) = a_side.await.unwrap();
    b.await.unwrap();
    Report::ok(impl_name(), &a, n, start.elapsed(), token).with_latencies(&mut samples)
}
...
/// Sends the token `n` times, waiting for each reply; times every `every`-th round trip.
async fn ping_mpsc(to_b: mpsc::Sender<u64>, mut a_rx: mpsc::Receiver<u64>, n: u64, every: u64, mut samples: Vec<u64>) -> (u64, Vec<u64>) {
    let mut token = 0u64;
    for i in 0..n {
        let t0 = (i % every == 0).then(Instant::now);
        to_b.send(token + 1).await.unwrap();
...
/// Replies to every token with token + 1 until `ping` hangs up.
async fn pong_mpsc(mut b_rx: mpsc::Receiver<u64>, to_a: mpsc::Sender<u64>) {
...
/// Tokio has no multi-receiver channel, so MPMC uses async-channel, like `tokio-bench`.
async fn mpmc_async_channel(a: Args) -> Report {
    let per = a.size / a.producers as u64;
    let total = per * a.producers as u64;
    ...
    let start = Instant::now();
    for _ in 0..a.consumers {
        consumers.push(tokio::spawn(drain_sum_async_channel(rx.clone())));
    }
    drop(rx);
    for p in 0..a.producers as u64 {
        producers.push(tokio::spawn(produce_async_channel(tx.clone(), p * per..(p + 1) * per)));
    }
    drop(tx);
    for p in producers {
        p.await.unwrap();
...
async fn produce_async_channel(tx: async_channel::Sender<u64>, values: Range<u64>) {
    for i in values {
        tx.send(i).await.unwrap();
    }
}

/// `drain_sum` on an async-channel receiver.
async fn drain_sum_async_channel(rx: async_channel::Receiver<u64>) -> u64 {
    let mut sum = 0u64;
    while let Ok(v) = rx.recv().await {
        sum = sum.wrapping_add(v);
        for _ in 1..BATCH {
